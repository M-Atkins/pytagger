...
# /home/zeus/Documents/fromssd/taggerpy/image/fashion/a7db2a4b629a53647b38be377e2e121f_noface.jpg
...
def savetext():
    ...
    im = Image.open("/home/zeus/Documents/fromssd/taggerpy/image/landscapes/"+str(itm))
    imtxt = "/home/zeus/Documents/fromssd/taggerpy/image/landscapes/"+str(itm)[:-4] +str(".txt")
    f = open(imtxt,'r')
...
def appendall():
    print("running!")
    x = 0
    for f in listdir("image/fashion"):
        x + 1
        if isfile(join("image/fashion", f)) and ".txt" in f:
            print(f)
            inp = textSearch.get(1.0, "end")
            f = open(join("image/fashion", f),'a')
            f.write(","+ inp.rstrip())
            f.close()
...
im = Image.open("/home/zeus/Documents/fromssd/taggerpy/image/landscapes/img295.jpg")
...
x = 0
for f in listdir("image/landscapes"):
    x + 1
    if isfile(join("image/landscapes", f)) and ".jpg" in f:
        lb.insert(x, str(f))
...
savebutton = Button(right_frame,text="Save", command=savetext)
