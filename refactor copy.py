...
def getfiles():
    ...
    ##tried sorting, didn't work
    files.sort()
    for f in files:
        lb.insert(files.index(f), str(f))

##Only fix I can think of maybe
##Get list from firedir, then sort by alphabetical before inserting
##Get list from selection, also sort by alphabetical, then choose last item, might prevent automatic indexing and preserve memory of list.

###Compare two lists, one of the last selection, the other of the newest selection and display the element that does not yet match.
def compareLists(x):
    set_difference = set(x) - set(second_list)
...
def getSelection():
    ...
    set_difference = set(selected_text_list) - set(oldlist)

    oldlist = selected_text_list.copy()

    listitem = ' '.join(map(str, set_difference))
    print("++++++++")
    print(listitem)
    print("++++++++")
    return listitem
...
def getPath():
    ...
    impath = path + result
    print(impath)
    #print(selected_text_list[-1])
    #impath = path + selected_text_list[-1]
